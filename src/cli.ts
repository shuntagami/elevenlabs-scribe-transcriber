...
program
  ...
  .argument('<input-path>', '文字起こしする音声または動画ファイルのパス、もしくはYouTubeのURL')
  .option('-e, --no-audio-events', '音声イベントのタグ付けを無効にする (デフォルト: 有効)')
  .option('-f, --format <format>', '出力形式', /^(text|json)$/i, 'text')
  .option('-o, --output <file>', '出力ファイルのパス (指定しない場合は自動生成)')
  .option('--output-dir <dir>', '出力ディレクトリ (デフォルト: transcripts)', 'transcripts')
